...
public class SimplexNoise
{
	...
	// -------------------------------------------------------------------------------------
	// Range 0..1
	public static float fbmu(Vector3 _loc, int _octaves, float _lacunarity = 2.0321f, float _persistence = 0.5074f)
	{
		return MathUtils.Map(fbm(_loc, _octaves, _lacunarity, _persistence), -1f, 1f, 0f, 1f);
	}

	// -------------------------------------------------------------------------------------
	// Range -1..1
	public static float fbm(Vector3 _loc, int _octaves, float _lacunarity = 2.0321f, float _persistence = 0.5074f)
	{
		...
		for (int i = 0; i < _octaves; i++)
		{
			...
			float layerNoise = Noise(tmpLoc.x, tmpLoc.y, tmpLoc.z );
	...
	// -------------------------------------------------------------------------------------
	// Range 0..1
	public static float fbmu(Vector4 _loc, int _octaves, float _lacunarity = 2.0321f, float _persistence = 0.5074f)
	{
		return MathUtils.Map(fbm(_loc, _octaves, _lacunarity, _persistence), -1f, 1f, 0f, 1f);
	}
	...
	// -------------------------------------------------------------------------------------
	// Range -1..1
	public static float fbm(Vector4 _loc, int _octaves, float _lacunarity = 2.0321f, float _persistence = 0.5074f)
	{
		float finalNoise = 0.0f;
		float amplitude = 1.0f;
		float totalAmplitude = 0.0f;
		...
		for (int i = 0; i < _octaves; i++)
		{
			amplitude *= _persistence;
			totalAmplitude += amplitude;
			float layerNoise = Noise(tmpLoc.x, tmpLoc.y, tmpLoc.z, tmpLoc.w);
			finalNoise += layerNoise * amplitude;
			tmpLoc *= _lacunarity; // //sum += amp * snoise(pp);
		}
	...
	// ------------------------------------------------------------
	public static Vector3 noiseOffset(Vector3 _pos, float _time)
	{
		Vector3 vel = Vector3.zero;
		vel.x += Noise(_pos.x, _pos.y, _pos.z, _time);
		vel.y += Noise(_pos.y, _pos.z, _time, _pos.x);
		vel.z += Noise(_pos.z, _time, _pos.x, _pos.y);
		return vel;
	}

	// ------------------------------------------------------------
	public static Vector3 noiseFbmOffset(Vector3 _pos, float _time, int _octaves, float _lacunarity = 2.0321f, float _persistence = 0.5074f )
	{
		Vector3 vel = Vector3.zero;
		vel.x += fbm( new Vector4(_pos.x, _pos.y, _pos.z, _time), _octaves, _lacunarity, _persistence );
		vel.y += fbm( new Vector4(_pos.y, _pos.z, _time, _pos.x), _octaves, _lacunarity, _persistence);
		vel.z += fbm( new Vector4(_pos.z, _time, _pos.x, _pos.y), _octaves, _lacunarity, _persistence);
		return vel;
	}
