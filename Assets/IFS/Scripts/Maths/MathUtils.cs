...
public class MathUtils
{
    ...
    public static Vector3 RotatePointAroundLine(Vector3 lineA, Vector3 lineB, Vector3 point, float angle)
    {
        Vector3 rotationCenter = lineA + Vector3.Project(point - lineA, lineB - lineA);
        Vector3 rotationAxis = (lineB - lineA).normalized;
        Vector3 relativePosition = point - rotationCenter;

        Quaternion rotatedAngle = Quaternion.AngleAxis(angle, rotationAxis);
    ...
    public static void RotateTransformAroundLine(Vector3 lineA, Vector3 lineB, ref Transform transform, float angle)
    {
        Vector3 rotationCenter = lineA + Vector3.Project(transform.position - lineA, lineB - lineA);
        Vector3 rotationAxis = (lineB - lineA).normalized;
        transform.RotateAround(rotationCenter, rotationAxis, angle);
    }
    ...
    // ------------------------------------------------------------
    public static Vector3 closestPointOnRay(  Vector3 _rayStart, Vector3 _rayDir, Vector3 _point,  out float _t )
	{
		Vector3 startToPoint = _point - _rayStart;
		_t = Vector3.Dot( _rayDir, startToPoint );
		return _rayStart + (_t * _rayDir);
	}
		
	// ------------------------------------------------------------
	public static Vector3 closestPointOnLine( Vector3 _a, Vector3 _b, Vector3 _point, out float _t )
	{
		Vector3 ap = _point - _a;			// vector from a to point
		Vector3 ab = _b - _a;				// vector from a to b
		float d = ab.magnitude;				// = Distance( a, b);  // length Line
		...
		_t = Vector3.Dot( ab, ap );
			
		if( _t <= 0f ) { return _a; } // point projected on line ab is out of line, closest point on line is a
 ...
 */
    ...
    /// 
    /// </summary>
    /// <param name="x">Normalised input</param>
    /// <param name="a">Amount of easing. Below 0.5 easout, above ease in </param>
    /// <returns></returns>
